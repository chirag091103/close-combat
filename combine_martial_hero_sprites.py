...
def combine_martial_hero_sprites():
    pygame.init()
    ...
    sprite_files = [
        ...
        ("Attack1.png", 6),    # attack1 - 6 frames
        ("Attack2.png", 6),    # attack2 - 6 frames
        ("Take Hit.png", 4),   # hit - 4 frames
        ("Death.png", 6)       # death - 6 frames
    ]
    
    base_path = "assets/images/Martial Hero/Sprites"
    ...
    max_frames = 8  # Maximum frames from all animations
    ...
    pygame.image.save(combined, os.path.join(base_path, "martial_hero.png"))
    print(f"\nCreated sprite sheet: {max_frames * frame_size}x{len(sprite_files) * frame_size}")
    pygame.quit()

if __name__ == "__main__":
    combine_martial_hero_sprites() 